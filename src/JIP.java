...
public class JIP extends JFrame implements ActionListener {
	...
	protected void setupGUI() {
		...
		String[] searchPaths = System.getProperty("java.class.path", ".").split(File.pathSeparator);
		for (int i = 0; i < searchPaths.length; i++) {
			...
			for (int j = 0; j < classFiles.length; j++) {
				...
				try {
					...
					if (BoundaryFinder.class.isAssignableFrom(c)) {
						addEffect((BoundaryFinder)c.newInstance());
					}
				} catch (Exception e) {
				}
			}
...
saveOuter:
	...
	public void addEffect(BoundaryFinder ie) {
		JMenuItem menuItem = new JMenuItem(ie.getDescription());
		menuItem.addActionListener(this);

		int pos = 0;
		try {
			while (pos < effectsMenu.getMenuComponentCount() && 
					ie.getDescription().compareTo(((JMenuItem)effectsMenu.
							getMenuComponent(pos)).getText()) >= 0) {
					pos++;
			}
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == menuItemOpenImage) {
			...
			BoundaryFinder effect = (BoundaryFinder)effectMap.get(e.getSource());
			BufferedImage img = (BufferedImage)imagePanel.getBackgroundImage();
			if (effect != null && img  != null) {
				BufferedImage newImg = effect.apply(img);
...
public static String folder = "";
	public void promptOpenImage() {
		...
		folder = openFileChooser.getSelectedFile().getParent();
		if (img == null) {
